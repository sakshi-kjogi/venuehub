import { Link } from 'react-router-dom';
import { useVenuesQuery } from '@/features/venues/useVenueQueries';

export default function Venues() {
  const { data: venues, isLoading, isError } = useVenuesQuery();

  if (isLoading) return <div className="mt-16 text-center text-gray-500">Loading venues…</div>;
  if (isError) return <div className="mt-16 text-center text-red-600">Could not load venues.</div>;

  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <h1 className="text-3xl font-bold">Venues</h1>
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {venues?.map((venue) => (
          <Link
            key={venue.id}
            to={`/venues/${venue.id}`}
            className="block rounded-lg border border-gray-200 p-4 hover:shadow-md"
          >
            <h2 className="text-lg font-semibold">{venue.name}</h2>
            <p className="text-sm text-gray-500">{venue.city}, {venue.country}</p>
            <p className="mt-2 text-sm text-gray-700">Capacity: {venue.capacity}</p>
            <p className="mt-1 font-medium text-blue-600">₹{venue.pricePerDay.toLocaleString()} / day</p>
          </Link>
        ))}
        {venues?.length === 0 && <p className="text-gray-500">No venues published yet.</p>}
      </div>
    </div>
  );
}